...
export function RaffleAdmin() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div><p className="text-sm font-semibold uppercase text-primary">Evento en vivo</p><h2 className="font-serif text-3xl font-semibold">Sorteo</h2><p className="mt-2 text-muted-foreground">Prepará el sorteo y abrí la pantalla pública para proyectarla en el evento.</p></div>
      <form onSubmit={prepareRaffle} className="space-y-4 rounded-xl border p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="winner-count">Cantidad de ganadores</Label><Input id="winner-count" type="number" min="1" max="50" value={winnerCount} onChange={(event) => setWinnerCount(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="prize">Premio</Label><Input id="prize" placeholder="Clase privada, regalo..." value={prize} onChange={(event) => setPrize(event.target.value)} /></div></div>
        <Button type="submit" disabled={creating}>{creating ? 'Preparando…' : 'Cargar participantes'}</Button>
      </form>
